...
const Commercial = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/projects')
      ...
      .then(data => {
        const commercial = (data.data || []).filter(p => p.category === 'Commercial');
        setProjects(commercial);
        setLoading(false);
      })
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative px-6 md:px-12 pt-32 pb-24 overflow-hidden"
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <nav className="flex items-center gap-2 mb-10 text-xs uppercase tracking-widest font-ui">
            ...
            <span className="text-white/20">›</span>
            <span className="text-accent font-700">Commercial</span>
          </nav>
          ...
          <h1 className="text-white font-display font-700" style={{ fontSize: 'clamp(48px, 7vw, 100px)' }}>
            Offices That <span className="text-accent italic">Elevate</span>
          </h1>
          <p className="text-white/45 text-sm mt-4 max-w-md leading-relaxed font-body">
            Commercial construction and fitouts that reflect your company's vision, culture, and long-term ambitions.
          </p>

          {/* STATS */}
          <div className="flex gap-12 mt-12 pt-10 border-t border-white/10">
            {[['80+', 'Commercial Projects'], ['4', 'Cities Served'], ['100%', 'On-Time Delivery']].map(([num, label]) => (
              <div key={label}>
      ...
      <div style={{ background: '#f8f9fa', padding: '64px 48px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
          {[
            { icon: '🏢', title: 'Office Buildings', desc: 'Corporate towers and mid-rise office complexes built to last.' },
            { icon: '🏪', title: 'Retail Spaces', desc: 'Showrooms and retail fitouts that drive footfall and experience.' },
            { icon: '🏭', title: 'Industrial Facilities', desc: 'Warehouses and factories built for efficiency and safety.' },
            { icon: '🏨', title: 'Hospitality', desc: 'Hotels and restaurants crafted for guest experience.' },
          ].map((item, i) => (
      ...
      <section className="px-6 md:px-12 py-20 max-w-7xl mx-auto">
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-muted uppercase tracking-widest text-xs font-ui mb-4">No commercial projects found.</p>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0a0e17 0%, #0d1f35 100%)', padding: '80px 48px', textAlign: 'center' }}>
        <p style={{ fontFamily: "'Poppins'", fontSize: 10, fontWeight: 500, letterSpacing: '0.5em', textTransform: 'uppercase', color: '#00adee', marginBottom: 16 }}>Start a Commercial Project</p>
        <h2 style={{ fontFamily: "'Poppins'", fontWeight: 300, fontSize: 'clamp(28px, 4vw, 52px)', color: '#fff', margin: '0 0 32px' }}>
          Let's build your <span style={{ color: '#00adee', fontWeight: 700 }}>next workspace.</span>
        </h2>
        <Link to="/contact"
          style={{
