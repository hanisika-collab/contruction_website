...
const ImageUpload = ({ label, value, onChange }) => {
 ...
 // Replace the handleFile function inside ImageUpload component
...
const handleFile = (e) => {
  ...
  img.onload = () => {
    ...
    // Resize to max 1200px wide, compress to JPEG at 75% quality
    const MAX_W = 1200;
...
const WorksSection = ({ toast }) => {
  ...
  const [editing,       setEditing]       = useState(null); // project _id or null
...
const handleSave = async () => {
  ...
  const payload = {
    ...
    year:        form.year || null,
    images: {
      before: form.beforeImg || null,
      after:  form.afterImg  || null,
    },
  };

  try {
    const url    = editing
      ? `http://localhost:5000/api/projects/${editing}`
    ...
    // FIX 3: distinguish oversized-payload / connection reset from server-down
    if (
  ...
  /* ── setters ── */
  // FIX: separate setter for event-based inputs vs raw string values (images).
  // ImageUpload calls onChange(base64String) — not a synthetic event.
    ...
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Project' : 'Add New Project'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* IMAGE UPLOADS — FIX: use setImage() not setField() */}
...
══════════════════════════════════════════════════════════ */
const EMPTY_PACKAGE = {
  name: '', tagline: '', price: '', badge: '', featured: false, features: [''],
};
  ...
  return (
    <div>
      <SectionHeader
        ...
        action={<button onClick={openNew} style={btnPrimary}>+ Add Package</button>}
      />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
          {packages.map(pkg => (
            <div key={pkg.id} style={{ background: C.white, border: `1.5px solid ${pkg.featured ? C.accent : C.border}`, padding: 24, position: 'relative' }}>
              {pkg.badge && (
                <div style={{ position: 'absolute', top: -1, right: 20, background: C.accent, color: '#fff', fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '3px 12px' }}>
                  {pkg.badge}
                </div>
              )}
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {(pkg.features || []).slice(0, 4).map((f, i) => (
                  <li key={i} style={{ fontSize: 12, color: C.muted, display: 'flex', gap: 8 }}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Package' : 'Add Pricing Package'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <Field label="Badge (optional)" half>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <button onClick={addFeature} style={{ background: 'none', border: `1px solid ${C.accent}`, color: C.accent, fontSize: 11, fontWeight: 700, cursor: 'pointer', padding: '4px 10px' }}>
                + Add Feature
              </button>
            </div>
              ...
              {form.features.map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {form.features.length > 1 && (
                    <button onClick={() => removeFeature(i)} style={{ background: 'none', border: 'none', color: '#e53e3e', cursor: 'pointer', fontSize: 20, lineHeight: 1, flexShrink: 0 }}>×</button>
                  )}
                </div>
              ))}
            </div>
